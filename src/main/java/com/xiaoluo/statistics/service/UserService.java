...
public class UserService {
    ...
    public String getUid(String phone,String property,String value){
        String uid= userPropertyDao.getUidByProperty(property,value);
        //如果为空,则是一个新的设备上报
        if(uid==null){
            uid= UUIDKit.base58Uuid();
            userPropertyDao.insert(uid,phone,property,value);
        }
        return uid;
    }
    public UserBindResult bindUser(String phone,String property,String value){
        //注册或者登陆时才会触发
        UserBindResult result=new UserBindResult();
        if(phone!=null&&!phone.trim().equals("")){
            //旧的uid
            List<String> oldUids= userPropertyDao.getUidByPhone(phone);
            String newUid= getUid(phone,property,value);
            result.setOldUids(oldUids);
            result.setNewUid(newUid);
            if(result.needUpdate()){
                userPropertyDao.updateOldUid(oldUids,newUid);
            }
            userPropertyDao.updatePhone(phone,property,value);
            return result;
        }else{
